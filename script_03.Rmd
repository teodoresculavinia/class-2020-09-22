...
sep_old <- 
  ...
  summarize( enrollment = sum(u_grad))
...
Before we start on the model, Wisdom suggests we should:
 - What data we actually need (class enrollment from 9/1 to today)
 - Understand what data we actually have (it is only to 9/18)

After we have a model, Temperance suggests we should:
 - Understanding that the average drop in undergraduate enrollment might not reflect each individual course
 - It might be influenced by external factors such as COVID
 - Assuming this table has all courses/ 
 - We are not counting courses under 10

```{r}

joint_enrollment <-
  inner_join(by=c("id", "department", "title", "name"), x= sep_old, y=sep_new, suffix=c(".old", ".new")) %>%

  mutate(enrollment_difference = enrollment.new - enrollment.old)
